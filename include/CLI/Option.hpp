...
class OptionBase {
 ...
 protected:
  ...
  /// Policy for multiple arguments when `expected_ == 1`  (can be set on bool
  /// flags, too)
...
class Option : public OptionBase<Option> {
 ...
 protected:
  ...
  /// A human readable default value, usually only set if default is true in
  /// creation
  ...
  /// A private setting to allow args to not be able to accept incorrect
  /// expected values
 ...
 public:
  ...
  /// Gets a , sep list of names. Does not include the positional name if
  /// opt_only=true.
  std::string get_name(bool opt_only = false) const {
    std::vector<std::string> name_list;
    if (!opt_only && pname_.length() > 0) name_list.push_back(pname_);
  ...
  /// The first half of the help print, name plus default, etc. Setting opt_only
  /// to true avoids the positional name.
  ...
  std::string help_aftername() const {
    ...
    if (get_expected() != 0) {
      if (!typeval_.empty()) out << " " << typeval_;
      if (!defaultval_.empty()) out << "=" << defaultval_;
      if (get_expected() > 1) out << " x " << get_expected();
      if (get_expected() == -1) out << " ...";
    }
    if (!envname_.empty()) out << " (env:" << envname_ << ")";
    if (!requires_.empty()) {
      out << " Requires:";
      for (const Option *opt : requires_) out << " " << opt->get_name();
    }
    if (!excludes_.empty()) {
      out << " Excludes:";
      for (const Option *opt : excludes_) out << " " << opt->get_name();
    }
    return out.str();
  }
    ...
    if (!validators_.empty()) {
      for (std::string &result : results_)
        for (const std::function<std::string(std::string &)> &vali :
          ...
          if (!err_msg.empty()) throw ValidationError(single_name(), err_msg);
        }
    }
  ...
  bool operator==(const Option &other) const {
    for (const std::string &sname : snames_)
      if (other.check_sname(sname)) return true;
    for (const std::string &lname : lnames_)
      if (other.check_lname(lname)) return true;
    // We need to do the inverse, just in case we are ignore_case
    for (const std::string &sname : other.snames_)
      if (check_sname(sname)) return true;
    for (const std::string &lname : other.lnames_)
      if (check_lname(lname)) return true;
    return false;
  }
  ...
  /// Puts a result at the end, unless last_ is set, in which case it just keeps
  /// the last one
  ...
  /// Set a custom option, typestring, expected; locks changeable unless
  /// expected is -1
  void set_custom_option(std::string typeval, int expected = 1) {
    typeval_ = typeval;
    expected_ = expected;
    if (expected == 0) required_ = false;
