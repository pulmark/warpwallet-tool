 ...
 * description. The templated
 *  add_option methods make it easy to prepare options. Remember to call
 * `.start` before starting your
 * program, so that the options can be evaluated and the help option doesn't
 * accidentally run your program. */
class App {
  friend Option;
 ...
 protected:
  ...
  ///  If true, return immediately on an unrecognised option (implies
  ///  allow_extras) INHERITABLE
  bool prefix_command_{false};

  /// This is a function that runs when complete. Great for subcommands. Can
  /// throw.
  ...
  /// True if ini is required (throws if not present), if false simply keep
  /// going.
 ...
 public:
  ...
  /// name.
  /// After start is called, you can use count to see if the value was passed,
  /// and
  /// the value will be initialized properly. Numbers, vectors, and strings are
  /// supported.
  ///
  ...
  /// Add option for non-vectors (duplicate copy needed without defaulted to
  /// avoid `iostream << value`)
  template <typename T,
  ...
  Option *add_option(std::string name,
    ...
    Option *opt = add_option(name, fun, description, defaulted);
    opt->set_custom_option(detail::type_name<T>(), -1);
    if (defaulted) opt->set_default_str("[" + detail::join(variable) + "]");
    return opt;
  }
  ...
  /// Bool version - defaults to allowing multiple passings, but can be forced
  /// to one if
  ...
  Option *add_set(std::string name,
    ...
    CLI::callback_t fun = [&member, options, simple_name](CLI::results_t res) {
      ...
      if (!retval) throw ConversionError(res[0], simple_name);
  ...
  Option *set_config(std::string name = "", std::string default_filename = "",
    ...
    if (config_ptr_ != nullptr) remove_option(config_ptr_);
  ...
  App *get_subcommand(App *subcom) const {
    for (const App_p &subcomptr : subcommands_)
      if (subcomptr.get() == subcom) return subcom;
  ...
  App *get_subcommand(std::string subcom) const {
    for (const App_p &subcomptr : subcommands_)
      if (subcomptr->check_name(subcom)) return subcomptr.get();
  ...
  /// parsing by default, in command line
  /// order)
  ...
  /// of the App. Set default_also to include
  /// default arguments. Prefix will add a string to the beginning of each
  /// option.
  ...
  std::string help(size_t wid = 30, std::string prev = "") const {
    ...
    for (const Option_p &opt : options_)
      if (opt->get_positional()) {
        ...
        if (opt->_has_help_positional()) pos = true;
      }
    ...
    if (pos) {
      ...
      for (const Option_p &opt : options_) {
        if (detail::to_lower(opt->get_group()).empty()) continue;  // Hidden
    ...
    if (npos) {
      for (const std::string &group : groups) {
        if (detail::to_lower(group).empty()) continue;  // Hidden
 ...
 protected:
  ...
  void _validate() const {
    ...
    if (count > 1) throw InvalidError(name_);
    for (const App_p &app : subcommands_) app->_validate();
  }
  ...
  bool _valid_subcommand(const std::string &current) const {
    ...
    for (const App_p &com : subcommands_)
      if (com->check_name(current) && !*com) return true;
  ...
  detail::Classifer _recognize(const std::string &current) const {
    ...
    if (current == "--") return detail::Classifer::POSITIONAL_MARK;
    if (_valid_subcommand(current)) return detail::Classifer::SUBCOMMAND;
  ...
  void _parse(std::vector<std::string> &args) {
    ...
    for (const Option_p &opt : options_) {
      ...
      if (opt->get_required() || opt->count() != 0) {
        // Make sure enough -N arguments parsed (+N is already handled in
        // parsing function)
        if (opt->get_expected() < 0 &&
  ...
  /// Parse one ini param, return false if not found in any subcommand, remove
  /// if it is
  ///
  /// If this has more than one dot.separated.name, go into the subcommand
  /// matching it
  /// Returns true if it managed to find the option, if false you'll need to
  /// remove the arg manually.
  bool _parse_ini(std::vector<detail::ini_ret_t> &args) {
    ...
    if (!parent.empty()) {
      current.level++;
      for (const App_p &com : subcommands_)
        if (com->check_name(parent)) return com->_parse_ini(args);
      return false;
    }
    ...
    if (op_ptr == std::end(options_)) return false;
        ...
        if (current.inputs.size() == 1) {
          ...
          if (val == "true" || val == "on" || val == "yes")
            ...
            try {
              ...
              for (size_t i = 0; i < ui; i++) op->results_.emplace_back("");
  ...
  /// fails, add to missing if missing from
  /// master
    ...
    if (num < 0) {
      ...
      while (!args.empty() &&
        ...
        if (collected >= -num) {
          ...
          if (_count_remaining_positionals() > 0) break;
