...
/// using e.exit_code or as a quick shortcut,
...
/// Does not output a diagnostic in CLI11_PARSE, but allows to return from
/// main() with a specific error code.
  ...
  ArgumentMismatch(std::string name, int expected, size_t recieved)
      : ArgumentMismatch(expected > 0
                             ...
                             : ("Expected at least " +
                                ...
                                name + ", got " + std::to_string(recieved)),
...
/// should not ever see it
/// Strings are directly added to this error, but again, it should never be
/// seen.
